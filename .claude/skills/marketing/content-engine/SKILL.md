...
description: |
  ...
  publica en LinkedIn vía Unipile (multi-cuenta, recomendado) o en cualquier
  red vía Upload-Post API como fallback. Reemplaza workflows visuales de n8n
  por una sola conversación.
version: 0.3.0
...
| `UNIPILE_API_KEY` + `UNIPILE_BASE_URL` | Publicar en LinkedIn (multi-cuenta) vía Unipile | Path primario si están configuradas. Soporta solo LinkedIn. |
| `UPLOAD_POST_API_KEY` | Publicar en Instagram / X / TikTok / etc. | Fallback. Solo necesario si publicas fuera de LinkedIn o no usas Unipile. |
| `OPENAI_API_KEY` | Imagen vía OpenAI (`gpt-image-1`) | Solo si el usuario eligió OpenAI como proveedor |

**Lógica de publicación (selección automática)**:
- Si `UNIPILE_API_KEY` está set y la plataforma es `linkedin` → usa `tools/unipile_post.py`.
- En cualquier otra plataforma o si Unipile no está configurado → usa `tools/upload_post.py`.
- Si ninguna está configurada → la skill genera contenido pero no publica (modo solo-generación).
...
**Router de publicación** — para cada plataforma, elige el cliente:

| Plataforma | Cliente | Notas |
|---|---|---|
| `linkedin` | `tools/unipile_post.py` si `UNIPILE_API_KEY` está set, si no `tools/upload_post.py` | Unipile soporta múltiples cuentas LinkedIn (un `account_id` por marca/cliente). |
| `instagram`, `x`, `tiktok`, `facebook`, `threads`, `pinterest`, `bluesky`, `reddit`, `google_business` | `tools/upload_post.py` | Unipile no las cubre. |
| `youtube` (short) | (no soportado) | Ni Upload-Post ni Unipile publican shorts. Solo entregas el guion al usuario. |

##### Path A — Unipile (LinkedIn)

Lee del brief el campo `linkedin_account_id` (si existe). Si no, lista las
cuentas disponibles con `python tools/unipile_post.py --list-accounts` y
pregunta al usuario cuál usar. Guarda la elección en el brief para futuras
sesiones.

```bash
python tools/unipile_post.py \
    --account-id "<linkedin_account_id del brief>" \
    --text "$(cat data/inbox-redes/<YYYYMMDD>-<slug>/linkedin.md)" \
    --image data/inbox-redes/<YYYYMMDD>-<slug>/imagen-1.png
```

Si el usuario publica como **página de empresa** en lugar de perfil personal,
añade `--as-organization <linkedin_page_id>` (sacado del brief o preguntado).

Si la respuesta es 201 → muestra el `id` del post devuelto por Unipile y
construye la URL `https://www.linkedin.com/feed/update/<id>/` (formato típico,
puede variar).

##### Path B — Upload-Post (resto de plataformas)
...
Llama `tools/upload_post.py` UNA SOLA VEZ con todas las plataformas
no-LinkedIn elegidas (la API soporta multi-plataforma):

```bash
python tools/upload_post.py \
    --user "<user del config>" \
    --image data/inbox-redes/<YYYYMMDD>-<slug>/imagen-1.png \
    --title "<caption del canal principal>" \
...
Guarda el resultado de ambos paths en
`data/inbox-redes/<YYYYMMDD>-<slug>/publish-log.json` con timestamp,
plataforma, cliente usado (unipile|upload_post), account_id (si aplica),
post_id devuelto y url construida.
